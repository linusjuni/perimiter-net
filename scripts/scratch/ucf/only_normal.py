...
# --- CONFIGURATION ---
TARGET_SOURCE = "/dtu/blackhole/10/187952/ucf-crime-blackhole/Normal_Videos_for_Event_Recognition"
...
def process_video_loud(video_path, output_root):
    ...
    if not cap.isOpened():
        print(f"❌ FAILED (Could not open video)")
        return

    # 4. Read First Frame (The critical test)
    ret, frame = cap.read()
    if not ret:
        print(f"❌ FAILED (Opened, but frame 0 read returned False)")
        return
